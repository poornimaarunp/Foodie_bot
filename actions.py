...
class ActionValidateLocation(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		try:
			if location is None and freetext is not None and str(freetext).lower() not in tier1:
				dispatcher.utter_template("utter_wrong_city",tracker)
				location = None
			elif location is None or str(location).lower() not in tier1:
				dispatcher.utter_template("utter_wrong_city",tracker)
				location = None
...
class ActionSendEmail(Action):
	...
	def run(self, dispatcher, tracker, domain):
		...
		if messageBody is not None:
			...
			dispatcher.utter_message("No results fetched to send email. Please retry changing your preference")
...
class ZomatoClient:
	...
	def restaurantResultForEmail(self,dispatcher, tracker, domain,maxcount):
		...
		try:
			...
			messageBody = messageBody + "<tr> <th>Restaurant</th> <th>Address</th>  <th>Average price for two</th> <th>Zomato User Rating</th> </tr>"
				...
				if d['results_found'] == 0:
					...
					for restaurant in d['restaurants']:
						try:
							...
							rating = restaurant['restaurant']['user_rating']['aggregate_rating']
							if minbudget is not None and maxbudget is not None and price <= int(maxbudget) and price >= int(minbudget):
								messageBody = messageBody + " <tr> <td>"+ restaurant['restaurant']['name']+ "</td> <td>"+ restaurant['restaurant']['location']['address']+"</td> <td>"+str(price)+"</td> <td>"+str(rating)+"</td></tr>"
								count = count + 1
							elif minbudget is None and maxbudget is not None and price <= int(maxbudget):
								messageBody = messageBody + " <tr> <td>"+ restaurant['restaurant']['name']+ "</td> <td>"+ restaurant['restaurant']['location']['address']+"</td> <td>"+str(price)+"</td> <td>"+str(rating)+"</td></tr>"
								count = count + 1
